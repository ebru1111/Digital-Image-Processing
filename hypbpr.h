void RGB2HHYPbPr(struct ppm_file *image,struct YPbPr *ypbpr); //Converts from RGB into HYPbPr
void HYPbPr2RGB(struct ppm_file *image,struct YPbPr *ypbpr); //Converts from HYPbPr into RGB
void HYPbPrEqualization(char *fname); //HYPbPr Equalization
//Converts from RGB into HYPbPr (HDTV)
void RGB2HYPbPr(struct ppm_file *image,struct YPbPr *ypbpr)
{
	int k,height,width;
	float r,g,b;
	...
	for(k=0;k<height*width;k++)
	{
		...
		g=image->gdata[k];
		b=image->bdata[k];
		ypbpr->Y[k]=round(0.213*r+0.715*g+0.072*b);
		ypbpr->Pb[k]=-0.115*r-0.385*g+0.5*b;
		ypbpr->Pr[k]=0.5*r-0.454*g-0.046*b;
	}
	return;
	
}
//Converts from HYPbPr (HDTV) into RGB
void HYPbPr2RGB(struct ppm_file *image,struct YPbPr *ypbpr)
{
	int k;
	float r,g,b;
...
//HYPbPr (HDTV) Equalization
void HYPbPrEqualization(char *fname)
{
